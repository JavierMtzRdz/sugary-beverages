...
Poisson GLME models exhibit overdispersion for this data (mean being less than variance). Therefore, the results above are obtained using negative binomial GLME, which eliminates overdispersion. In addition, negative binomial models have better QQ plots, which is illustrated in @fig-pois and @fig-nb.
...
For the second question regarding the difference in intervention effects between sites, the results are summarized in Table @fig-q2. It is interesting to note that discount plus messaging has different effect across sites for both zero-calorie and sugary beverages.
